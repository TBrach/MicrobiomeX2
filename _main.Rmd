...
- What then?
    - Adjust the input parameters for the analysis under **Load phyloseq object and set input**. I hope most input variables are self-explanatory in particular with the annotation given in the section. As a help: all lines where user input is required or possible are marked with **NB: USER INPUT** and **NB: user input**, respectively
       - NB: In the section: **Load phyloseq object and set input**, I intentionally put the input tests directly after the input is given. That way it takes longer to write the input. I did so anyway, because I think it is a good idea to go once through the entire section for each analysis. It helps in understanding the input and discovering potential problems.
- How to run it?
...
- This is a basic pipeline that can be extented to more complicated scenarios. **This basic pipeline only allows the comparison of TWO groups**! I.e. it compares the microbiome samples of one group to the samples of another group. It can be extended to more groups by loops, see: *example in progress*.
...
- the user further defines the *taxonomic_level* at which the analysis will be performed. The phyloseq object will be adjusted to the chosen level with the phyloseq::taxglom command.
...
## Removal of taxa that were not assigned to phylum level (if chosen) 
...
<!--chapter:end:010_ExploratoryAnalysisofPhyloseqObject.Rmd-->
...
<!--chapter:end:020_Alpha_diversity_analysis.Rmd-->
...
<!--chapter:end:030_Preprocessing_filtering.Rmd-->
...
<!--chapter:end:040_Beta_diversity_analysis.Rmd-->
...
<!--chapter:end:050_Differential_abundance_analysis.Rmd-->
