...
### Visualise the effect of the size factor adjustment

- the plot shows the count distribution of the samples before and after size factor adjustment.
- *NB: the plot can take time, but I think it is useful (it takes more time the more taxa you have, so tax_glom to phylum would speed it up). In this plot only the samples included in group_var_levels are shown, but remember size factors were calculated based on all samples!*

```{r, echo = FALSE, message=FALSE}
# - generate barplot representations of counts before and after total count adjustment - 
bar_plot_ps_vs_ps_tca <- plot_sample_bars_compare(physeq = ps, physeq2 = ps_tca, x = "Sample", y = "Abundance", group_var = group_var, color_levels = color_levels, color_sample_names = TRUE, fill = "Phylum", col_vec = phylum_colors)
# --
```
